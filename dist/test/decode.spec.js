...
describe('#decode', () => {
    ...
    it('should decode a complex object', () => {
        ...
        chai_1.expect(obj).to.be.ok;
        chai_1.expect(obj.int8Test).to.equal(0x7F);
        chai_1.expect(obj.uint8Test).to.equal(0xFF);
        chai_1.expect(obj.int16leTest).to.equal(0x7FFF);
        chai_1.expect(obj.int16beTest).to.equal(0x7FFF);
        chai_1.expect(obj.uint16leTest).to.equal(0xFFFF);
        chai_1.expect(obj.uint16beTest).to.equal(0xFFFF);
        chai_1.expect(obj.int32leTest).to.equal(0x7FFFFFFF);
        chai_1.expect(obj.int32beTest).to.equal(0x7FFFFFFF);
        chai_1.expect(obj.uint32leTest).to.equal(0xFFFFFFFF);
        chai_1.expect(obj.uint32beTest).to.equal(0xFFFFFFFF);
        chai_1.expect(obj.float32leTest).to.be.closeTo(Math.PI, EPSILON);
        chai_1.expect(obj.float32beTest).to.be.closeTo(Math.PI, EPSILON);
        chai_1.expect(obj.float64leTest).to.be.closeTo(Math.PI, EPSILON);
        chai_1.expect(obj.float64beTest).to.be.closeTo(Math.PI, EPSILON);
    });
    it('should decode UTF-8 string', () => {
        ...
        chai_1.expect(obj).to.be.ok;
        chai_1.expect(obj.stringUtf8Test).to.equal(stringValue);
    });
    it('should decode UTF-16 string', () => {
        ...
        chai_1.expect(obj).to.be.ok;
        chai_1.expect(obj.stringUtf16Test).to.equal(stringValue);
    });
    it('should decode a nested object', () => {
        ...
        const obj = lib_1.BufferCodec.from(buffer).parse({
            test1: 'uint8',
        ...
        chai_1.expect(obj).to.be.ok;
        chai_1.expect(obj.test1).to.equal(0x01);
        chai_1.expect(obj.test2).to.equal(0xFFFF);
        chai_1.expect(obj.inner).to.be.ok;
        chai_1.expect(obj.inner.test3).to.equal(0x7FFFFFFF);
    });
    it('should decode an array', () => {
        const objects = [1, 2, 3].map(number => ({ id: number, value: Math.pow(number, 2) }));
        ...
        const result = lib_1.BufferCodec.from(buffer).parse([{
                id: 'uint8',
                value: 'uint16|littleEndian'
            }]);
        chai_1.expect(result).to.be.ok;
        chai_1.expect(result).to.deep.equal(objects);
    });
});
