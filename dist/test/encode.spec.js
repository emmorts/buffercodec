...
describe('#encode', () => {
    ...
    it('should encode UTF-8 string', () => {
        const now = new Date().toString();
        const codec = new lib_1.BufferCodec({ encoding: 'utf8' });
        const buffer = codec.string(now).result();
    ...
    it('should encode float32le', () => {
        const codec = new lib_1.BufferCodec();
        const buffer = codec.float32(1.1 * Math.pow(10, 38), true).result();
        const expectedBuffer = new ArrayBuffer(4);
        const expectedBufferView = new DataView(expectedBuffer);
        expectedBufferView.setFloat32(0, 1.1 * Math.pow(10, 38), true);
        const areEqual = utils_1.areBuffersEqual(buffer, expectedBuffer);
        chai_1.expect(areEqual).to.be.true;
    });
    it('should encode float32be', () => {
        const codec = new lib_1.BufferCodec();
        const buffer = codec.float32(1.1 * Math.pow(10, 38)).result();
        const expectedBuffer = new ArrayBuffer(4);
        const expectedBufferView = new DataView(expectedBuffer);
        expectedBufferView.setFloat32(0, 1.1 * Math.pow(10, 38), false);
        const areEqual = utils_1.areBuffersEqual(buffer, expectedBuffer);
        chai_1.expect(areEqual).to.be.true;
    });
    it('should encode float64le', () => {
        const codec = new lib_1.BufferCodec();
        const buffer = codec.float64(1.1 * Math.pow(10, 300), true).result();
        const expectedBuffer = new ArrayBuffer(8);
        const expectedBufferView = new DataView(expectedBuffer);
        expectedBufferView.setFloat64(0, 1.1 * Math.pow(10, 300), true);
        const areEqual = utils_1.areBuffersEqual(buffer, expectedBuffer);
        chai_1.expect(areEqual).to.be.true;
    });
    it('should encode float64be', () => {
        const codec = new lib_1.BufferCodec();
        const buffer = codec.float64(1.1 * Math.pow(10, 300)).result();
        const expectedBuffer = new ArrayBuffer(8);
        const expectedBufferView = new DataView(expectedBuffer);
        expectedBufferView.setFloat64(0, 1.1 * Math.pow(10, 300), false);
        const areEqual = utils_1.areBuffersEqual(buffer, expectedBuffer);
        chai_1.expect(areEqual).to.be.true;
    });
